...
//		}
    ...
    void RequestStartQuest() {
        ...
        StartQuest startQuest = startQuestNetBase as StartQuest;
        startQuest.Param = p;
        startQuestNetBase.OnRequest(RspStartQuest);
    }

    void RspStartQuest(ErrorMsg errMsg) {
        LogHelper.Log("rspStartQuest code:{0}, error:{1}", errMsg.code, errMsg.Msg);

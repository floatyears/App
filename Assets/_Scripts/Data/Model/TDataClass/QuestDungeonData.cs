	...
	public void assignData() {
		...
		for(int nFloor=0; nFloor < floors.Count; nFloor++){
			...
			for(int f=0; f< floors[nFloor].gridInfo.Count; f++){
				QuestGrid grid = new QuestGrid (floors[nFloor].gridInfo[f]);
				...
				for(int i=0; i<drop.Count;i++){
					...
					if ( drop[i].dropId == grid.dropId ){
						LogHelper.Log ("===floor[{0}].Add => grid[{1}/{2}].dropId=={3} unitId:{4}",
						               nFloor,i,floor.Count, drop[i].dropId, drop[i].unitId);
						grid.Drop = drop [i];
						break;
					}
				}
				...
				for(int g=0; g<grid.enemyId.Count;g++){
					for(int i=0; i<enemys.Count;i++){
						if ( grid.enemyId[g] == enemys[i].enemyId ){
							LogHelper.Log ("grid[{0}]: assign enemy[{1}], enemyCount={2}...  ", g, grid.enemyId[g], grid.Enemy.Count);
...
//				LogHelper.Log ("===floor[{0}].Add grid{1}", nFloor,floor.Count);
	...
	public List<EnemyInfo>	Boss {get { return boss;} 
			set { 
				boss.Clear();
				boss.AddRange(value);
			} 
...
public partial class QuestGrid : ProtoBuf.IExtensible {
	...
	public QuestGrid(QuestGrid inst) { 
//		LogHelper.LogError ("TQuestGrid :: instance.trap id : " + instance.trapId);
	...
	public TrapBase	TrapInfo { get { 
			if (!DataCenter.Instance.TrapInfo.ContainsKey(trapId) ) {
//				UnityEngine.Debug.LogError("instance.trapId : " + instance.trapId);
			...
			return DataCenter.Instance.TrapInfo[trapId]; 
		} 
	}
}
...
public partial class DropUnit : ProtoBuf.IExtensible {
	...
	public UnitInfo UnitInfo {
		get { return DataCenter.Instance.GetUnitInfo (unitId); }
	}
}
}
